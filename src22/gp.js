...
exports.mutShrink = exports.mutInsert = exports.mutNodeReplacement = exports.mutUniform = exports.cxOnePointLeafBiased = exports.cxOnePoint = exports.generate = exports.genHalfAndHalf = exports.genGrow = exports.genFull = void 0;
 ...
 */
function genFull(pset, min, max, type) {
    function condition(height, depth) {
        return height === depth;
    }
    return generate(pset, min, max, condition, type);
}
exports.genFull = genFull;
/**
 ...
 * @returns A list of leaves representing the expression.
 */
function genHalfAndHalf(pset, min, max, type) {
    return random.random() < 0.5 ? genGrow(pset, min, max, type) : genFull(pset, min, max, type);
}
exports.genHalfAndHalf = genHalfAndHalf;
/**
 ...
 * @returns A list of leaves representing the expression.
 */
function generate(pset, min, max, condition, ret_type) {
    if (ret_type === undefined)
        ret_type = pset.ret_type;
    let expr = [];
    const height = random.randInt(min, max);
    let stack = [[0, ret_type]];
...
function cxOnePoint(ind1, ind2) {
    if (ind1.length < 2 || ind2.length < 2)
        return [ind1, ind2]; // No crossover on single-node trees
    let compatible_idx = getCompatibleIdx(ind1, ind2);
    if (compatible_idx.length === 0)
        return [ind1, ind2];
 ...
 * @returns
 */
function cxOnePointLeafBiased(ind1, ind2, termpb) {
    if (ind1.length === 0 || ind2.length === 0)
        return [ind1, ind2]; // No crossover on single-node trees
    let terminal_op = (a) => a === 0;
    let primitive_op = (a) => 0 < a;
...
exports.cxOnePointLeafBiased = cxOnePointLeafBiased;
// Mutation functions // --------------------------------------------------------------------------------
/**
 ...
 */
function mutUniform(individual, expr) {
 ...
 * @returns The mutated individual
 */
function mutNodeReplacement(individual, pset) {
    if (individual.length < 2)
        return individual; // No mutation on single-node trees
...
exports.mutNodeReplacement = mutNodeReplacement;
//TODO: Finish implement mutInsert
function mutInsert(individual, pset) {
...
exports.mutInsert = mutInsert;
function mutShrink(individual, pset) {
    if (individual.length < 2)
        return individual; // No mutation on single-node trees
    let iprims = [];
...
*/
function getCompatibleIdx(ind1, ind2, cond1, cond2) {
    ...
    for (let [idx1, ret_type1, parent_in_type1] of types1) {
        for (let [idx2, ret_type2, parent_in_type2] of types2) {
            if (ret_type1.isCompatible(parent_in_type2) &&
